...
test("delete all transactions API", async ({ browser }) => {
  ...
  await page.goto("https:/test.systemaml.pl/dashboard/transactions/");
  await page.waitForLoadState('networkidle');

  const rowsPerPageSelector = page.locator('[aria-label="page number"]');
  await rowsPerPageSelector.click();
  await page.locator('li[role="option"]:has-text("25")').click();
  ...
  const collectTransactionCodes = async () => {
    ...
    while (true) {
      ...
      const currentPageCodes = await rows.locator('td.MuiTableCell-root:nth-child(1)')
  ...
  const apiContext = await request.newContext({
    baseURL: 'https://apitest.systemaml.pl',
    extraHTTPHeaders: {
